...
# Det kan være vi vil produsere flere slike plot. Da er det greit å automatisere
# med en funksjon, som ligner på det vi ville gjort i GeoGebra

def tegnGraf(funksjon,fra,til,inndeling=100):
    fig=plt.figure()
    ax=plt.axes()
    x=np.linspace(fra,til,(til-fra)*100)
    plt.plot(x,funksjon(x))
    plt.show()
 
# Nå kan vi tegne grafen bed å skrive inn:
# tegnGraf(restmengde,0,10)
# Vi ser at de to grafene blir like
...
print("Oppskrift hentet fra: Eksempel hentet fra: https://docs.scipy.org/doc/scipy/reference/optimize.nonlin.html")

# fsolve finner nullpunkt. Vi må derfor skrive om uttrykket vårt fra
# restmengde(tid)=400
# til 
# restmengde(tid)-400 =0
# og lagre vestresiden som en ny funksjon
...
# vi kunne lagt til en logisk funksjon som ville gitt et annet svar dersom
# vi ikke fant noen løsning.
...
print("Oppskrift hentet fra: https://docs.scipy.org/doc/scipy-0.15.1/reference/generated/scipy.misc.derivative.html")
